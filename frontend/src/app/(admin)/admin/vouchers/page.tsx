...
import {
  Plus, Search, Pencil, Trash2, X, Ticket,
  Percent, Banknote, Calendar, Hash, ToggleLeft, ToggleRight
} from "lucide-react";
import { formatCurrency, formatDate, formatDateTime } from "@/lib/utils";
import { VOUCHER_STATUS_LABELS, VOUCHER_STATUS_COLORS, VOUCHER_DISCOUNT_TYPE_LABELS } from "@/lib/constants";
import { getAdminVouchers, createVoucher, deleteVoucher } from "@/lib/api/vouchers.api";
import { Voucher, VoucherDiscountType, CreateVoucherData } from "@/types/voucher.types";
...
export default function AdminVouchersPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editingVoucher) {
        setVouchers((prev) => prev.map((v) => (v.id === editingVoucher.id ? { ...v, ...form, updatedAt: new Date().toISOString() } : v)));
      } else {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-heading text-3xl font-bold text-foreground mb-2">Quản lý Voucher</h1>
        ...
        <div className="glass-card overflow-hidden">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/30">
                  <th className="text-left px-6 py-4 text-sm font-medium text-muted-foreground">Mã</th>
                  <th className="text-left px-6 py-4 text-sm font-medium text-muted-foreground">Tên voucher</th>
                  <th className="text-left px-6 py-4 text-sm font-medium text-muted-foreground">Giảm giá</th>
                  <th className="text-left px-6 py-4 text-sm font-medium text-muted-foreground">Đã dùng</th>
                  <th className="text-left px-6 py-4 text-sm font-medium text-muted-foreground">Trạng thái</th>
                  <th className="text-left px-6 py-4 text-sm font-medium text-muted-foreground">Hết hạn</th>
              ...
              <tbody>
                {filtered.map((voucher) => {
                  ...
                  return (
                    <tr key={voucher.id} className="border-b border-white/20 hover:bg-white/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <p className="text-xs text-muted-foreground truncate max-w-[200px]">{voucher.description}</p>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${VOUCHER_STATUS_COLORS[status]}`}>
          ...
          {filtered.length === 0 && (
            <div className="p-12 text-center">
              ...
              <p className="text-muted-foreground text-sm">Không tìm thấy voucher nào</p>
            </div>
          )}
        </div>
      )}
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowModal(false)} className="p-2 rounded-xl hover:bg-white/40 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              {/* Code */}
              <div>
                <label className="block text-sm font-medium text-foreground mb-1.5">Mã voucher</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">Tên voucher</label>
                  <input
                    type="text"
                    value={form.name}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">Loại giảm giá</label>
                  <select
                  ...
                  >
                    ...
                    <option value={VoucherDiscountType.FIXED}>Giảm cố định (VNĐ)</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">
                    Giá trị {form.discountType === VoucherDiscountType.PERCENT ? "(%)" : "(VNĐ)"}
                  </label>
                  <input
                    type="number"
                    value={form.discountValue || ""}
                    onChange={(e) => setForm({ ...form, discountValue: Number(e.target.value) })}
                    placeholder={form.discountType === VoucherDiscountType.PERCENT ? "VD: 20" : "VD: 50000"}
                    required
                    min={1}
                    max={form.discountType === VoucherDiscountType.PERCENT ? 100 : undefined}
                    className="w-full px-4 py-3 rounded-xl bg-white/60 border border-white/50 focus:outline-none focus:ring-2 focus:ring-violet-300 text-sm"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">Giảm tối đa (VNĐ)</label>
                  <input
                    type="number"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">Đơn tối thiểu (VNĐ)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">Giới hạn lượt sử dụng</label>
                  <input
                    type="number"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="date"
                      value={typeof form.startDate === "string" ? form.startDate.split("T")[0] : form.startDate}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="date"
                      value={typeof form.endDate === "string" ? form.endDate.split("T")[0] : form.endDate}
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-white/40 border border-white/50">
                <div>
                  <p className="text-sm font-medium text-foreground">Kích hoạt voucher</p>
                  <p className="text-xs text-muted-foreground mt-0.5">Voucher sẽ hiển thị cho khách hàng khi được bật</p>
                </div>
                <button
                  type="button"
