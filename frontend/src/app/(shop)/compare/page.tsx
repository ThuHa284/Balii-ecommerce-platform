      ...
      <div className="max-w-4xl mx-auto px-4 py-16 text-center space-y-6 min-h-[60vh] flex flex-col justify-center items-center">
        ...
        <div className="space-y-2 max-w-md">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            Bạn chưa chọn sản phẩm nào để so sánh. Hãy quay lại cửa hàng, nhấn
            biểu tượng đối chiếu trên các sản phẩm để thêm vào bảng so sánh.
          </p>
        </div>
        <Link
          href="/products"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/20 pb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            So sánh chi tiết thông tin, kích thước và giá cả của các sản phẩm
            lụa ngủ Balii.
          </p>
        </div>
        <button
        ...
        <table className="w-full border-collapse text-left min-w-[640px]">
          <thead>
            <tr className="border-b border-violet-100/80">
              <th className="p-6 text-sm font-bold text-slate-500 w-1/4 bg-slate-50/20">
                Thuộc tính
              </th>
              ...
              {Array.from({ length: 3 - compareItems.length }).map((_, idx) => (
                <th
                  key={idx}
                  className="p-6 w-1/4 bg-slate-50/5 text-center text-slate-400 font-medium text-xs"
                >
          ...
          <tbody className="divide-y divide-violet-100/50">
            {/* Price Row */}
            <tr>
              <td className="p-6 text-xs font-bold text-slate-500 bg-slate-50/20">
                Giá bán lẻ
              </td>
              {compareItems.map((product) => {
            ...
            <tr>
              ...
              {compareItems.map((product) => (
                <td
                  key={product.id}
                  className="p-6 text-xs text-slate-700 leading-relaxed font-medium"
                >
                  ...
                  <td key={product.id} className="p-6">
                    <div className="flex flex-wrap gap-1.5">
                      {sizes.map((s) => (
                        <span
                          key={s}
                          className="px-2.5 py-1 bg-violet-50 text-violet-700 text-xs font-bold rounded-lg border border-violet-100"
                        >
                          {s}
                        </span>
                      ))}
                ...
                <td key={product.id} className="p-6">
                  <div className="space-y-1 bg-violet-50/30 p-3 rounded-xl border border-violet-100 text-[10px] font-semibold text-slate-700">
                    {Array.from(
                      ...
                      <div
                        key={sizeLabel}
                        className="flex justify-between border-b border-slate-100/50 pb-0.5 last:border-b-0"
                      >
                        <span>Size:</span>
            ...
            <tr>
              ...
              {compareItems.map((product) => {
                const colors = Array.from(
                  new Map(
                    product.variants.map((v) => [v.color, v.colorCode]),
                  ).entries(),
                );
                return (
                  <td key={product.id} className="p-6">
            ...
            {/* Purchase Action Row */}
            <tr>
              <td className="p-6 text-xs font-bold text-slate-500 bg-slate-50/20 bg-violet-500/5">
                Hành động
              </td>
              {compareItems.map((product) => (
