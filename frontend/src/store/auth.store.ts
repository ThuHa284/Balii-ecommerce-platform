...
export const useAuthStore = create<AuthState>()((set, get) => ({
  ...
  addAddress: async (addressData) => {
    ...
    const fallbackIds =
      addressData.districtId && addressData.wardId
        ? null
        : await resolveFallbackLocationIds(addressData.provinceId);
    ...
    const updatedAddresses =
      shouldSetDefault || addresses.length === 0
        ? [...addresses.map((a) => ({ ...a, isDefault: false })), savedAddress]
  ...
  updateAddress: async (id, data) => {
    const fallbackIds =
      data.provinceId != null && (!data.districtId || !data.wardId)
