import { Voucher, UserVoucher, CreateVoucherData, VoucherDiscountType } from "@/types/voucher.types";
...
export async function getAvailableVouchers(): Promise<Voucher[]> {
  if (USE_MOCK) {
    return new Promise((resolve) =>
      setTimeout(() => resolve(mockVouchers.filter((v) => v.isActive && new Date(v.endDate) > new Date() && v.usedCount < v.usageLimit)), 500)
    );
  }
  return [];
}
...
export async function getAdminVouchers(): Promise<Voucher[]> {
  if (USE_MOCK) {
    return new Promise((resolve) =>
      setTimeout(() => resolve(mockVouchers), 500)
    );
  }
  return [];
}
...
export async function updateVoucher(id: string, data: Partial<CreateVoucherData>): Promise<Voucher> {
  if (USE_MOCK) {
    const voucher = mockVouchers.find((v) => v.id === id);
    if (!voucher) throw new Error("Voucher không tồn tại");
    const updated = { ...voucher, ...data, updatedAt: new Date().toISOString() };
